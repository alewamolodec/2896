...
import lms.core.person.workemail.ConnectedWorkMailBean;
...
import org.mirapolis.service.mail.EmailSenderInfo;
...
import static org.mirapolis.mvc.view.clientscript.builders.ComboBoxExpressionBuilder.createExpressionForDisableFields;
import static org.mirapolis.mvc.view.clientscript.builders.ComboBoxExpressionBuilder.createExpressionForShowFields;
...
public class PersonBean extends ReflectDataBean implements UserData, PersonExtension {
	...
	/**
	 * @return Возвращает последнюю локаль пользователя или локаль по-умолчанию из настроек, если у пользователя локаль
	 *         пустая
	 */
	...
	public boolean isAnonym() {
		return PersonStatus.ANONYM == status;
	}
	...
	public boolean isActive() {
		return PersonStatus.ACTIVE == status;
	}
	...
	public boolean isArchive() {
		return PersonStatus.ARCHIVE == status;
	}
	...
	/**
	 * Создать информацию об отправителе письма на основе данного физлица. Если заданы «Личные настройки подключения к
	 * почтовому серверу», то берем данные оттуда, иначе из PersonContactBean и ФИО физлица.
	 */
	public EmailSenderInfo createEmailSenderInfo() {
		ConnectedWorkMailBean workMailFilter = new ConnectedWorkMailBean();
		workMailFilter.setPersonId(id);
		return EntityManager.findOptional(workMailFilter)
			.filter(workMail -> isNotEmpty(workMail.getEmail()))
			.map(
				workMail -> new EmailSenderInfo(
					workMail.getEmail(),
					workMail.getPassword(),
					StringHelper.defaultIfEmpty(workMail.getSenderName(), getFullName()),
					true
				)
			)
			.orElseGet(
				() -> new EmailSenderInfo(getContactBean().getEmail(), getFullName())
			);
	}
	...
	public String getLastNameWithInitial() {
		return getFirstName().charAt(0) + "." + getSurName().charAt(0) + "." + getLastName();
	}
	...
	public static DataObject createDataObject() throws CoreException {
		DataObject dataObject = new DataObject(DATANAME, VVMessage.person, PersonBean.class).setFields(
			...
			new StringField(FIO, PersonMessage.fullname, FIO_LENGTH).setIsName().setIndex().enableUpperCaseIndex(),
			...
			new ComboField(STATUS, PersonMessage.status, PersonStatus.class).setIndex(),
			new StringField(QP_ID, PersonMessage.query).setIsHidden(),
			new StringField(QC_ID, PersonMessage.query).setIsHidden(),
			new StringField(ME_ID, PersonMessage.measure).setIsHidden(),
			new BeanField(PERSON_MAIN_WORK, PersonMessage.aw_work, PersonWorkBean.DATANAME, "").setIndex(),
			new StringField(AUTONUMBER, VVMessage.autonumber).setIsHidden(),
			Service.hasModule(AccountModule.class)
				? new LookupField(DATA_ACCOUNT_ID, PersonMessage.account, AccountFrame.NAME).setIsHidden()
				: null,
			Service.hasModule(AccountModule.class)
				? new DoubleField(BALANCE, AccountMessage.balance).setIsHidden()
				: null,
			...
			Service.hasModule(QuaModule.class)
				...
				.setScript(createExpressionForShowFields(PersonProfileType.PERSONAL.getValue(), PROFILE_QUA_ID))
				.getDataField()
				: null,
			...
			Service.hasModule(ProcedureModule.class)
				...
				.setScript(createExpressionForShowFields(PersonProfileType.PERSONAL.getValue(), PROFILE_KPI_ID))
				.getDataField()
				: null,
			new StringField(CONTANCT_VACANCY_ID, VVMessage.vacancy).setIsHidden(),
			Service.hasModule(DevelopmentModule.class)
				? new CheckField(ALLOW_CREATE_PLAN_DEVELOPMENT, DevelopmentMessage.allow_create_development_plans)
				: null,
	...
	private static ComboField createActualLocationDetTypeField() {
		...
		List<String> readOnlyValues = Arrays.asList(
		...
		return new ComboFieldBuilder(field)
			.setScript(createExpressionForDisableFields(readOnlyValues, ACTUAL_LOCATION))
			.getDataField();
	}
	...
	public void fillDefault() {
		super.fillDefault();
		getProfile().setBlog(PersonBlogVisibilityLevel.CUT_OFF);
